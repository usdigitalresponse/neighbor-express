import { 
	...
	TextButton,
	useGlobalConfig,
	useBase } from '@airtable/blocks/ui';
...
function FieldSetter({label, description, keyOrPath, tableName, ...setterProps}) {
	const globalConfig = useGlobalConfig();
  const base = useBase();
    ...
    <>
      ...
      {isDialogOpen && (
        <Dialog onClose={() => setIsDialogOpen(false)} width="320px">
          ...
          <FormField label="Name" description="A short descriptive name of the new type of email"> 
				  	<Input 
				  		value={name}
...
function EmailTypeSettings({emailType}) {
  ...
  const triggerField = deliveriesTable.getFieldIfExists(globalConfig.get('trigger_column'));
    ...
    <TextButton 
      ...
      Delete
		...
		<Accordion title={emailType}>
			...
			<FormField label="Send to..." description="Who should this email be sent to?">
				<SelectButtonsSynced
			...
			<FormField label="Send when..." description={`Email will be sent when ${triggerField.name} column is...`}>
				<SelectSynced
...
function AddTemplateVariableDialog({table}) {
  ...
  return (
    <>
      <Button onClick={() => setIsDialogOpen(true)}>Add new template variable</Button>
      {isDialogOpen && (
        <Dialog onClose={() => setIsDialogOpen(false)} width="320px">
          <Dialog.CloseButton />
		...
		<Box padding={3}>
			...
			<ul>
				{Object.keys(globalConfig.get(['template_variables', table.name])).map((f_id) => {
					...
					const sendgridValue = globalConfig.get(['template_variables', table.name, f_id]);
					const sendgridFormat = `{{${tableNameToSendgrid[tableName]}.${sendgridValue}}}`;
					const removeLink = (
						<TextButton 
				      onClick={() => {globalConfig.setAsync(['template_variables', table.name, f_id], undefined)}}>
				      (remove)
				    </TextButton>
					)
